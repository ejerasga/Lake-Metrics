...
def measure_object_size(image_path):
    ...
    if len(contours) > 0:
        ...
        scale_factor = 190# Adjust this based on your calibration
...
def upload_file():
    ...
    if file:
        ...
        if size is not None:
            return render_template('laguna.html', size=size)
        else:
            return render_template('laguna.html', error='No object detected in the uploaded image.')

if __name__ == '__main__':
    wood.run(port= 5008, debug=True)
